...
<Steps>
  ...
  <Step title="Query the response">
    You can use a `Template Column` as input to the `Query JSON` column with dynamic index to get the nth image: `images[n]`

    Alternatively, query all `images` and use `Single Value` column to get the chosen image based on nth index.
